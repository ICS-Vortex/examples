...
const TopAerobaticsPilots = ({server, header, tour, type}) => {
    const [pilots, setPilots] = React.useState([]);

    useEffect(() => {
        fetch(process.env.REACT_APP_API_HOST + `/api/open/servers/${server.id}/aebobatics-top-pilots?tour=${tour}?limit=10&type=${type}`)
            .then(r => r.json())
            .then(data => {
                if (data) {
                    setPilots(data);
...
export default TopAerobaticsPilots;